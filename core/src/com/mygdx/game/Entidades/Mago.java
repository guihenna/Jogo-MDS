...
public class Mago extends Personagem {
    public Mago(Boolean resVeneno, int[] modificadores) {
        ...
        TextureRegion[][] andando = TextureRegion.split(new Texture("mago_andando.jpeg"), SIZE, SIZE);
        for(int i = 0; i < 4; i++)
            rolls[i] = new Animation(SPEED, andando[i]); //0=UP,1=LEFT,2=DOWN,3=RIGHT
        TextureRegion[][] atacando = TextureRegion.split(new Texture("mago_atacando.jpeg"), SIZE, SIZE);
        for(int i = 4; i < 8; i++)
            rolls[i] = new Animation(SPEED, atacando[i-4]); //0=UP,1=LEFT,2=DOWN,3=RIGHT
        TextureRegion[][] magia = TextureRegion.split(new Texture("mago_castando.jpeg"), SIZE, SIZE);
        for(int i = 8; i < 12; i++)
            rolls[i] = new Animation(SPEED, magia[i-8]); //0=UP,1=LEFT,2=DOWN,3=RIGHT
        TextureRegion[][] morte  = TextureRegion.split(new Texture("mago_morrendo.jpeg"), SIZE, SIZE);
